...
class UpdateTaskUseCase {
    ...
    execute(params) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = params, newTask = __rest(params, ["id"]);
            const existingTask = yield this.repository.getById(id);
            if (!existingTask) {
                throw new Error('Task not found to update');
            }
            Object.assign(existingTask, newTask);
            return yield this.repository.update(id, new task_1.Task(existingTask));
        });
    }
}
